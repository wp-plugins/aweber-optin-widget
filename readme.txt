=== Simple Aweber Optin Widget - Lite ===
Contributors: absikandar
Tags: aweber, optin, widget, responsive, forms, email, design, autoresponder, newsletter, conversion, leads, list, sidebar, subscriber, contacts
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=AEBXZCEYG3DQC
Requires at least: 3.0.1
Tested up to: 4.0
Stable tag: 1.2
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Add an Aweber optin form widget to your wordpress site. Very easy to setup. Responsive and Customizations Ready.

== Description ==
Custom Widget that adds an aweber optin form on site. You can add an unlimited number of forms using this widget plugin. The form design and look can be easily controlled in the widget settings. Its Responsive , very Easy to Use and comes with Powerful Features. The aweber forms created by this widget plugin are generated locally and do not load javascripts and css from aweber server.


<strong>What's included in Lite Version?</strong>
<ul>
<li>Custom Title and Subtitle</li>
<li>Ability to change button color</li>
<li>Ability to change button text color</li>
<li>Custom Thank you page url</li>
<li>Custom redirect url for already subscribed users</li>
<li>Custom Placeholder text for Name Field</li>
<li>Custom Placeholder text for Email Field</li>
<li>Custom Button Text</li>
</ul>

<strong>Use the CSS selectors below to override default form styles in Lite Version:</strong>

    .twb_widget 
    .twb_name,
    .twb_email
    .twb_btn

<hr>

<strong>Pro Version is also available ($7 only)</strong>

<strong>What's included in Pro Version?</strong>

<p>Pro version includes all the features that are listed in Lite Version <strong>PLUS</strong></p>

<ul>
<li>Ability to change Main Background Color</li>
<li>Ability to change Title and Subtitle Colors</li>
<li>Ability to change Title Font Size</li>
<li>Ability to change Privacy Policy Text</li>
<li>Ability to change Privacy Policy Text Color</li>
<li>Ability to link optins with a specific form in Aweber</li>
<li>Ability to add meta tracking ID</li>
<li>Multiple Forms on single page</li>
<li>Premium Support</li>
</ul>
    

<strong>Live Demos:</strong>  http://theweb-designs.com/plugins/
<strong>Purchase Pro Version</strong> http://theweb-designs.com/plugins/


== Installation ==
Install the plugin and activate. Then go to wordpress widgets and there you will see the Aweber Optin Widget

== Screenshots ==
1. Screenshot
2. Screenshot
3. Screenshot
4. Screenshot
5. Screenshot